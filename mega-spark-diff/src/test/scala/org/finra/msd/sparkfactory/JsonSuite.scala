...
class JsonSuite extends SparkFunSuite {

  val directory = "json"
  val expectedSourceType = SourceType.JSON

  FileHelper.getFilenames(directory, "JsonTest", ".json")
    .foreach(filename => {
      test("SparkFactory Json test - " + filename) {
        ...
        val appleTable = SparkFactory.parallelizeJSONSource(jsonPath, "json_test", schema.fieldNames)
        assert(appleTable.getSourceType.equals(expectedSourceType))

        helpers.compareJsonDataFrameActualToExpected(appleTable, jsonPath, txtPath, Seq("key1", "key2"), ";")
      }
    })
}
